...
reverseRange arr r@(start, end) = ixmap b reverseIx arr
  where
    ...
    reverseIx i
      ...
      | start > end,
        inRange (start, high) i || inRange (low, end) i =
...
knotRanges b@(low, high) counts =
  ...
  where
    addMod x y = (x + y) `mod` rangeSize b
...
hashString = xorEach 16 . hash arr . deriveKey
  where
